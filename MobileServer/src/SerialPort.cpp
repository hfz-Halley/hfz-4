...
SerialPort::SerialPort() : _baudRate(-1),
...
SerialPort::SerialPort(std::size_t bufferSize) : _baudRate(-1),
												 _flowControl(FLOW_NONE),
												 _logger(Poco::Logger::get(LOGGER_NAME)),
												 _buffer(bufferSize),
												 _end(_buffer.begin()),
...
SerialPort::SerialPort(Poco::Logger &logger, std::size_t bufferSize) : _baudRate(-1),
																	   _flowControl(FLOW_NONE),
																	   _logger(logger),
																	   _buffer(bufferSize),
																	   _end(_buffer.begin()),
...
SerialPort::SerialPort(const std::string &device, int baudRate, const std::string &parameters, FlowControl flowControl, std::size_t bufferSize) : _baudRate(-1),
																																				  _flowControl(FLOW_NONE),
																																				  _logger(Poco::Logger::get(LOGGER_NAME)),
																																				  _buffer(bufferSize),
																																				  _end(_buffer.begin()),
...
std::size_t SerialPort::write(const char *data, std::size_t size)
{
	...
	std::size_t n = writeImpl(data, size);
	if (n < 0)
