...
static PQCQMIMSG ComposeQMUXMsg(UCHAR QMIType, USHORT Type, CUSTOMQMUX customQmuxMsgFunction, void *arg)
{
    ...
    pRequest->QMIHdr.Length = cpu_to_le16(le16_to_cpu(pRequest->MUXMsg.QMUXMsgHdr.Length) + sizeof(QCQMUX_MSG_HDR) + sizeof(QCQMUX_HDR) + sizeof(QCQMI_HDR) - 1);
...
static USHORT WdsStartNwInterfaceReq(PQMUX_MSG pMUXMsg, void *arg)
{
    ...
    //Set Profile Index
    if (profile->pdp && !s_is_cdma)
    { //cdma only allow one pdp
...
{
    if ((pRequest->QMIHdr.QMIType == pResponse->QMIHdr.QMIType) && (pRequest->QMIHdr.ClientId == pResponse->QMIHdr.ClientId))
    {
...
int QmiThreadSendQMITimeout(PQCQMIMSG pRequest, PQCQMIMSG *ppResponse, unsigned msecs)
{
    ...
    pthread_mutex_lock(&s_commandmutex); //阻塞上锁
    ...
    if (!strncmp("/dev/qcqmi2", "/dev/qcqmi", strlen("/dev/qcqmi")))
...
int QmiThreadRecvQMI(void *arg, PQCQMIMSG pResponse)
{
    ...
    if (s_pRequest && is_response(s_pRequest, pResponse))
    {
        free(s_pRequest);
        s_pRequest = NULL;
        s_pResponse = (PQCQMIMSG)malloc(le16_to_cpu(pResponse->QMIHdr.Length) + 1);
        if (s_pResponse != NULL)
        {
    ...
    else if ((pResponse->QMIHdr.QMIType == QMUX_TYPE_NAS) && (le16_to_cpu(pResponse->MUXMsg.QMUXMsgHdrResp.Type) == QMINAS_SERVING_SYSTEM_IND))
    {
        pthread_mutex_unlock(&s_commandmutex);
        return RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED;
    }
    else if ((pResponse->QMIHdr.QMIType == QMUX_TYPE_WDS) && (le16_to_cpu(pResponse->MUXMsg.QMUXMsgHdrResp.Type) == QMIWDS_GET_PKT_SRVC_STATUS_IND))
    {
        pthread_mutex_unlock(&s_commandmutex);
        return RIL_UNSOL_DATA_CALL_LIST_CHANGED;
    }
    else if ((pResponse->QMIHdr.QMIType == QMUX_TYPE_NAS) && (le16_to_cpu(pResponse->MUXMsg.QMUXMsgHdrResp.Type) == QMINAS_SYS_INFO_IND))
    {
        pthread_mutex_unlock(&s_commandmutex);
        return RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED; //如网络状态发生变化时，CP会上报RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED，
    }
    else
    {
...
struct wwan_data_class_str class2str[] = {
    {WWAN_DATA_CLASS_NONE, (CHAR *)"UNKNOWN"},
    {WWAN_DATA_CLASS_GPRS, (CHAR *)"GPRS"},
    {WWAN_DATA_CLASS_EDGE, (CHAR *)"EDGE"},
    {WWAN_DATA_CLASS_UMTS, (CHAR *)"UMTS"},
    {WWAN_DATA_CLASS_HSDPA, (CHAR *)"HSDPA"},
    {WWAN_DATA_CLASS_HSUPA, (CHAR *)"HSUPA"},
    {WWAN_DATA_CLASS_LTE, (CHAR *)"LTE"},
    {WWAN_DATA_CLASS_1XRTT, (CHAR *)"1XRTT"},
    {WWAN_DATA_CLASS_1XEVDO, (CHAR *)"1XEVDO"},
    {WWAN_DATA_CLASS_1XEVDO_REVA, (CHAR *)"1XEVDO_REVA"},
    {WWAN_DATA_CLASS_1XEVDV, (CHAR *)"1XEVDV"},
    {WWAN_DATA_CLASS_3XRTT, (CHAR *)"3XRTT"},
    {WWAN_DATA_CLASS_1XEVDO_REVB, (CHAR *)"1XEVDO_REVB"},
    {WWAN_DATA_CLASS_UMB, (CHAR *)"UMB"},
    {WWAN_DATA_CLASS_CUSTOM, (CHAR *)"CUSTOM"},
};

CHAR *wwan_data_class2str(ULONG type)
{
    unsigned int i = 0;
    for (i = 0; i < sizeof(class2str) / sizeof(class2str[0]); i++)
    {
        if (class2str[i].type == type)
        {
...
int requestRegistrationState2(UCHAR *pPSAttachedState, CHAR *CapStr)
{
    PQCQMIMSG pRequest;
    PQCQMIMSG pResponse;
    PQMUX_MSG pMUXMsg;
	...
	*/
    ...
    memcpy(CapStr, pDataCapStr, strlen(pDataCapStr));
...
int requestRegistrationState(UCHAR *pPSAttachedState, CHAR *CapStr)
{
    PQCQMIMSG pRequest;
    PQCQMIMSG pResponse;
    PQMUX_MSG pMUXMsg;
    ...
    if (s_9x07)
    {
        ...
        return requestRegistrationState2(pPSAttachedState, CapStr);
    }
...
int requestQueryDataCall(UCHAR *pConnectionStatus, int curIpFamily)
{
    ...
    *pConnectionStatus = QWDS_PKT_DATA_DISCONNECTED;
    ...
    if (oldConnectionStatus != *pConnectionStatus || 0)
    {
...
__requestSetupDataCall:
    ...
    if (le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXResult) || le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXError))
    {
        ...
        if (curIpFamily == IpFamilyV4 && old_auth == profile->auth && profile->user && profile->user[0] && profile->password && profile->password[0])
        {
...
int requestBaseBandVersion(const char **pp_reversion)
{
    ...
    if (revId && le16_to_cpu(revId->TLVLength))
    {
        ...
        if (s_9x07 == -1)
        { //fail to get QMUX_TYPE_WDS_ADMIN
